...
}`,cli_command:`claude mcp add twilio -- npx -y @twilio-alpha/mcp ACCOUNT_SID/API_KEY:API_SECRET`,info_url:`https://github.com/twilio-labs/mcp`}},cli:{install_win:`npm install -g twilio-cli`,install_mac:`brew tap twilio/brew && brew install twilio`,commands:[{label:`로그인`,code:`twilio login`},{label:`전화번호 목록`,code:`twilio phone-numbers:list`},{label:`SMS 발송`,code:`twilio api:core:messages:create --from +1xxx --to +82xxx --body "Hello"`},{label:`이벤트 로그`,code:`twilio logs:list`}],info_url:`https://www.twilio.com/docs/twilio-cli`}},{name:`DeepL API`,url:`https://deepl.com/pro-api`,desc:`고품질 번역 API. 한국어 포함 30개 언어·높은 자연스러움`,badge:`freepaid`,detail:{intro:`신경망 기반 고품질 번역 API. 구글 번역 대비 자연스러운 문장 품질로 유명하며, 한국어·일본어·독일어 등 30개 언어를 지원한다. 문서 파일 번역과 용어집 기능도 제공한다.`,features:[`30개 언어 상호 번역 (한국어 포함)`,`형식 유지 문서 번역 — PDF·Word·PPTX·XLIFF`,`용어집(Glossary) — 특정 단어를 지정 번역어로 고정`,`공식성 수준 설정 (격식체·비격식체)`,`Python·Node.js·PHP·Ruby·Java SDK`],usage:`deepl.com/pro에서 API 플랜 구독. 무료 플랜은 월 500,000자 제한. API 키 발급 후 SDK 또는 REST API 호출.`,notes:`기술 문서·마케팅 카피 번역 품질이 특히 우수. 대량 번역 시 배치 처리보다 문단 단위 분할 발송이 품질 안정적.`,mcp:{desktop_config:`{
  "mcpServers": {
    "deepl": {
      "command": "npx",
