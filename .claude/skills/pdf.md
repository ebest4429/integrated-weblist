---
name: pdf
description: 사용자가 PDF 파일과 관련된 모든 작업을 원할 때 이 스킬을 사용합니다. PDF에서 텍스트/표 읽기 또는 추출, 여러 PDF를 하나로 결합/병합, PDF 분할, 페이지 회전, 워터마크 추가, 새 PDF 생성, PDF 양식 작성, PDF 암호화/복호화, 이미지 추출, 스캔된 PDF의 OCR로 검색 가능하게 만들기 등이 포함됩니다. 사용자가 .pdf 파일을 언급하거나 PDF 생성을 요청하면 이 스킬을 사용합니다.
license: 독점 소유권. LICENSE.txt에 전체 조건이 있습니다
---
...
이 가이드는 Python 라이브러리와 명령줄 도구를 사용한 필수 PDF 처리 작업을 다룹니다. 고급 기능, JavaScript 라이브러리, 상세한 예제는 REFERENCE.md를 참조하세요. PDF 양식을 작성해야 하는 경우 FORMS.md를 읽고 지침을 따르세요.
...
**중요**: ReportLab PDF에서 유니코드 아래 첨자/위 첨자 문자(₀₁₂₃₄₅₆₇₈₉, ⁰¹²³⁴⁵⁶⁷⁸⁹)를 사용하지 마세요. 내장 폰트에 이 글리프가 없어 검은 사각형으로 렌더링됩니다.
...
Paragraph 객체가 아닌 캔버스 텍스트의 경우, 유니코드 아래 첨자/위 첨자 대신 폰트 크기와 위치를 수동으로 조정하세요.
...
# output_prefix-000.jpg, output_prefix-001.jpg 등으로 모든 이미지가 추출됩니다
```
