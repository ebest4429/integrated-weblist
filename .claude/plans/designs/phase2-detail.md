# Phase 2 사전설계 — detail 필드 순차 구현

> 상태: **사전설계** (`plans/designs/` 폴더) — Phase 1 완료 후 정식 활성화
> 활성화 절차: WORKSPACE.md 이름 변경 → 새 WORKSPACE.md → 마스터플랜 포인터 수정 → 이 파일을 `plans/phase2-detail.md`로 이동
...
| 항목 | 결정 내용 |
|------|---------|
| detail 필드 성격 | optional — 없는 서비스는 기존 desc만 표시 (하위 호환) |
| 작업 단위 | 세션당 카테고리 1개 (10~24개 서비스) |
| 소스 파일 분리 | master-source.md → 그룹별 5개 파일 분리 (AI 생략 방지) |
| 검증 방식 | verify-detail.py 스크립트 + master-source 완료 마커 열 |

---

## 파일 분리 계획

현재 `.Source-Files/master-source.md` (단일 338개) 분리:
...
> 구글 생태계(120개)는 세션당 1~2개 카테고리 단위로 작업
...
### 1. master-source 완료 마커 열

각 그룹 파일의 서비스 테이블에 `detail` 상태 열 추가:
...
- ✅ = JSON에 detail 필드 작성 완료
- 🔲 = 미작성

### 2. verify-detail.py 스크립트

`scripts/verify-detail.py`:
- 전체 `src/data/*.json` 순회
- 카테고리별 `detail` 필드 유무 집계
- 완료율 출력 (예: `🤖 AI 플랫폼·챗봇: 12/18 완료`)
- 세션 시작/종료 시 실행하여 실제 상태 확인
