...
- ✅ 전체 서비스 CLI 보유 여부 전수 조사 (34개 확인)
- ✅ 기존 서비스 cli 필드 추가 (6개 JSON 업데이트: database·mcp-automation·ai-platforms·api·dev-tools·hosting-infra)
- ✅ 신규 CLI 서비스 항목 추가 — 해당 없음 (기존 서비스 내 전부 커버)
- ✅ 빌드 확인 (경고 없음)
- ✅ 커밋 + 배포 + 샘플 검증
...
- [x] P5-1 UI 구현 완료 (가상 필터뷰 동작 확인)
- [x] MCP 연결 가상 뷰 정상 동작 확인 (기존 데이터 활용)
- [x] CLI 도구 가상 뷰 정상 동작 확인
- [x] CLI 데이터 조사 완료 + JSON 업데이트
- [x] 빌드 경고 없음
